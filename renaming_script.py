...
if getenv("platform") == "apptest":
    base_url = "https://cascadeapptest.csi.edu:8443/api/v1"
    print("Using apptest")
elif getenv("platform") == "prod":
    print("Using prod")
...
if getenv("cascade_site") is not None:
    sitename = getenv("cascade_site")
    sitename = str(sitename)
else:
    raise EnvironmentError("Please make sure cascade_site is included in the .env")
            ...
            for row in reader:
                try:
                    ...
                    if "metadata" in asset and "dynamicFields" in asset["metadata"]:
                        ...
                        asset.update(displayName=new_name)
                        ...
                        for relation_object in relationships_result["subscribers"]:
                            ...
                            r_asset_type = relation_object['type'] # prevent conflict with the asset_type used for images
                        ...
                        raise RuntimeError("asset does not have metadata")
